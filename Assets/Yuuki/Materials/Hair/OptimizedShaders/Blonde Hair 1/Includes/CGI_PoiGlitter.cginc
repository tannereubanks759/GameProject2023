    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = frac(poiMesh.uv[0] + float4(0,0,0,0).xy * _Time.x) * float(2000);
        ...
        switch(float(0))
        {
            ...
            if(float(5) > 0)
            {
                randomRotation = randomFloat3WiggleRange(randoPoint, float(0));
            }
            else
            {
                randomRotation = randomFloat3Range(randoPoint, float(0));
            }
            ...
            finalGlitter = lerp(0, float(0.314) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.659)), poiCam.viewDir)), float(300)), 0);
            finalGlitter *= glitterAlpha;
            break;
            case 1:
            float offset = random(randoPoint);
            float brightness = sin((_Time.x + offset) * float(5)) * float(20) - (float(20) - 1);
            finalGlitter = max(float(0.314) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * float(0.08) * 10));
            break;
        }
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(26.4), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(26.4) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(26.4) * glitterTexture.a * glitterMask;
        }
    }
