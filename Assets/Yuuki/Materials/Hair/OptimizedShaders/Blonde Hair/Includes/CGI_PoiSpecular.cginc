    ...
    float anisoSpec1Alpha, float anisoSpec2Alpha, float4 specularTint, float specularMixAlbedoIntoTint, float4 specularMap, float3 specularLight, float3 lightDirection, float3 halfDir, float nDotL, float jitter, float4 packedTangentMap, in float4 albedo)
{
    ...
    //float3 normalDirection = normalize(mul(poiMesh.normals[float(0)], poiTData.tangentTransform));
...
float3 calculateSpecular(in float4 albedo)
{
...

    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.03310475,0.03310475,0.03310475,1), metallic, float(0.75) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(1) == 4)
    {
        ...
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.25,0.3,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(0)], poiLight.halfDir, poiLight.attenuation);
...

        {
            ...
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.75), float(0), float(1), float(1), float4(0.03310475,0.03310475,0.03310475,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(0.03310475,0.03310475,0.03310475,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.03310475,0.03310475,0.03310475,1), metallic, float(0.75) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
    ...
    finalSpecular *= specularMask;
...

        {
            ...
            finalSpecular1 = calculateNewSpecular(specularMap1.rgb, float(1), realisticAlbedo1, float4(0.03310475,0.03310475,0.03310475,1), metallic1, float(0.75) * specularMap1.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
        }
        ...
        if (float(1) == 4)
        {
            ...
            finalSpecular1 += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap1, poiLight.nDotL, float(0.95), float(1), float4(0.03310475,0.03310475,0.03310475,1), albedo, metallic1, jitter, float(0), packedTangentMap);
        ...
        {
            finalSpecular1 = calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.25,0.3,0,1), metallic1, float(1), specularMap1, specularLightColor, poiMesh.normals[float(0)], poiLight.halfDir, poiLight.attenuation);
            finalSpecular1 *= float4(0.03310475,0.03310475,0.03310475,1);
        }
...

        {
            ...
            finalSpecular1 = AnisotropicSpecular(float(0), float(0), float(0.75), float(0), float(1), float(1), float4(0.03310475,0.03310475,0.03310475,1), metallic1, specularMap1, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular1 *= float4(0.03310475,0.03310475,0.03310475,1);
            finalSpecular1 *= poiLight.attenuation;
        }
            ...
            // Non Important Lights
                ...
                for (int index = 0; index < 4; index++)
                {
                    ...
                    {
                        finalSpecular1 += calculateNewSpecular(specularMap1.rgb, float(1), realisticAlbedo1, float4(0.03310475,0.03310475,0.03310475,1), metallic1, float(0.75) * specularMap1.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                    }
                }
            #endif
        #endif
        
        finalSpecular1 *= float4(0.03310475,0.03310475,0.03310475,1).a;
