    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(0.002190549,0.002190549,0.002190549,1);
        #else
            matcap = float4(0.002190549,0.002190549,0.002190549,1);
        #endif
        matcap.rgb *= float(5);
        #if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
