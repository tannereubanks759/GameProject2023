...
void calculateAndApplyClearCoat(inout float4 finalColor)
{
	...
	float NormalDotView = abs(dot(float(0.824), _ClearCoatNormalToUse == 0 ? poiLight.N0DotV: poiLight.nDotV).r);
	#ifdef POI_LIGHTING
		finalColor.rgb = lerp(finalColor.rgb, reflectionColor * lerp(1, poiLight.finalLighting, lighty_clear_boy_uwu_var), clearCoatMap * float(0.824) * clamp(FresnelTerm(float(0.824), NormalDotView), 0, 1));
	...
	#else
		finalColor.rgb = lerp(finalColor.rgb, reflectionColor, clearCoatMap * float(0.824) * clamp(FresnelTerm(float(0.824), NormalDotView), 0, 1));
	#endif
}
*/
...
{
	float roughness = 1 - (float(1) * clearcoatMap.a);
	roughness = clamp(roughness, 0.0045, 1.0);
	roughness = roughness * roughness;
	
	float reflectivity = float(0.824) * clearcoatMap.r;
