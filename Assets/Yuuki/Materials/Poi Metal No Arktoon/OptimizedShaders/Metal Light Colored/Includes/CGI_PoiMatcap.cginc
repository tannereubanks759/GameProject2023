    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(0.0007301845,0.0007301845,0.0007301845,1);
        #else
            matcap = float4(0.0007301845,0.0007301845,0.0007301845,1);
        #endif

        matcap.rgb *= float(3.58);
        #if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
        ...
        blendMatcap(finalColor, float(0), float(0), float(1), matcap, matcapMask, matcapEmission, float(20)
        #ifdef POI_LIGHTING
        , float(0)
        #endif
        #ifdef POI_BLACKLIGHT
