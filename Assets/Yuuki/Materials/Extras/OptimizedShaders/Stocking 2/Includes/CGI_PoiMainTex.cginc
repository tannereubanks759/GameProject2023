...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        albedo = float4(mainTexture.rgb * max(float4(0.00477695,0.00477695,0.00477695,1).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, vertexColor, float(0)), mainTexture.a * max(float4(0.00477695,0.00477695,0.00477695,1).a, 0.0000001));
        ...
        applyBackFaceTexture(backFaceDetailIntensity, mixedHueShift, albedo, backFaceEmission);
            ...
            albedo.rgb = lerp(albedo.rgb, dot(albedo.rgb, float3(0.3, 0.59, 0.11)), -float(0.506) * hueShiftAlpha.b);
            ...
            #endif
            albedo.rgb *= LerpWhiteTo(detailTexture * float(0.8) * unity_ColorSpaceDouble.rgb, detailMask.r * float(1) * backFaceDetailIntensity);
        #endif
