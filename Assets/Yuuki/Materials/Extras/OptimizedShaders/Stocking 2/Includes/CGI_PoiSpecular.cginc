...
float3 calculateSpecular(in float4 albedo)
{
...

    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.03310475,0.03310475,0.03310475,1), metallic, float(-0.54) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(1) == 4)
    {
        ...
        finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.9), float(0.85), float4(0.03310475,0.03310475,0.03310475,1), albedo, metallic, jitter, float(0), packedTangentMap);
...

        {
            ...
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(-0.54), float(0), float(1), float(1), float4(0.03310475,0.03310475,0.03310475,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(0.03310475,0.03310475,0.03310475,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.03310475,0.03310475,0.03310475,1), metallic, float(-0.54) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
    ...
    finalSpecular *= specularMask;
        ...
        if (float(1) == 4)
        {
            ...
            finalSpecular1 += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap1, poiLight.nDotL, float(0.9), float(0.85), float4(0.03310475,0.03310475,0.03310475,1), albedo, metallic1, jitter, float(0), packedTangentMap);
