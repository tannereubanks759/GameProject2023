    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = frac(poiMesh.uv[0] + float4(0,0,0,0).xy * _Time.x) * float(3000);
        ...
        for (int j = -1; j <= 1; j ++)
        {
            for (int i = -1; i <= 1; i ++)
            {
                ...
                float2 rando = pos;
                pos = 0.5 + 0.5 * sin(float(0) * 6.2831 * pos);
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.686)), poiCam.viewDir)), float(948)), 0);
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(0.2), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(0.2) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(0.2) * glitterTexture.a * glitterMask;
        }
    }
