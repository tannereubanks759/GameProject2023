...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
        ...
        float rimNoise = 0;
    #endif
    rimNoise = (rimNoise - .5) * float(0.858);
    ...
    float rimBrighten = float(0.67);

    float rimWidth = lerp( - .05, 1, float(0.948));
    #ifdef POI_AUDIOLINK
    ...
    rimWidth = max(lerp(rimWidth, rimWidth * lerp(0, 1, poiLight.lightMap - float(0.144)) * float(10), float(0.097)), 0);
    rim = 1 - smoothstep(min(float(0.078), rimWidth), rimWidth, viewDotNormal);
