    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = frac(poiMesh.uv[0] + float4(0,0,0,0).xy * _Time.x) * float(1500);
        ...
        switch(float(0))
        {
            ...
            if(float(100) > 0)
            {
                randomRotation = randomFloat3WiggleRange(randoPoint, float(13.6));
            }
            else
            {
                randomRotation = randomFloat3Range(randoPoint, float(13.6));
            }
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.725)), poiCam.viewDir)), float(300)), 0);
            finalGlitter *= glitterAlpha;
            break;
            case 1:
            float offset = random(randoPoint);
            float brightness = sin((_Time.x + offset) * float(100)) * float(20) - (float(20) - 1);
        ...
        glitterColor *= lerp(1, albedo, float(0.708));
        #if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(2.9), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(2.9) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(2.9) * glitterTexture.a * glitterMask;
        }
    }
