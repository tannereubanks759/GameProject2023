...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimBrighten = float(0.66);

    float rimWidth = lerp( - .05, 1, float(0.6));
    #ifdef POI_AUDIOLINK
    ...
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.08393916,0.08393916,0.08393916,1);
    #else
        rimColor = float4(0.08393916,0.08393916,0.08393916,1);
    #endif
    ...
    rim = 1 - smoothstep(min(float(0.1), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0.08393916,0.08393916,0.08393916,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.053)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.053)) + lerp(albedo.rgb, rimColor, float(0.053)) * rimBrighten, rim);
}
#endif
