...
half2 _AudioLinkFlipbookFrame;
    ...
    void applyFlipbook(inout float4 finalColor, inout float3 flipbookEmission)
    {
            ...
            flipBookMask = 1;
        #endif
        float4 flipbookScaleOffset = float4(0.04,0.03,-0.012,0.04);
        ...
        float theta = radians(float(5) + _Time.z * float(0));
        float cs = cos(theta);
        float sn = sin(theta);
        ...
        if (float(0) == 0)
        {
        ...
        #if defined(PROP_FLIPBOOKTEXARRAY) || !defined(OPTIMIZER_ENABLED)
            float currentFrame = fmod(float(-1), float(21));
            if (float(-1) < 0)
            {
                currentFrame = (_Time.y / (1 / float(20))) % float(21);
            }
            #ifdef POI_AUDIOLINK
            ...
            if (float(0))
            {
                float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(21))));
        ...
        if (float(0))
        {
            flipBookPixel.rgb = float4(0.3229691,0.3229691,0.3229691,1).rgb;
        }
        else
        {
            flipBookPixel.rgb *= float4(0.3229691,0.3229691,0.3229691,1).rgb;
        ...
        finalColor.rgb = lerp(finalColor.rgb, flipBookPixel.rgb, flipBookPixel.a * float4(0.3229691,0.3229691,0.3229691,1).a * float(0) * flipBookMask * flipbookAlpha);
        finalColor.rgb = finalColor + flipBookPixel.rgb * float(1) * flipBookMask * flipbookAlpha;
        finalColor.rgb = finalColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(0.3229691,0.3229691,0.3229691,1).a * flipBookMask * float(0) * flipbookAlpha);
        
        
        if (float(0))
        {
            finalColor.a = lerp(finalColor.a, flipBookPixel.a * float4(0.3229691,0.3229691,0.3229691,1).a, flipBookMask);
        }
        ...
        #endif
        flipbookEmission = lerp(0, flipBookPixel.rgb * flipbookEmissionStrength, flipBookPixel.a * float4(0.3229691,0.3229691,0.3229691,1).a * flipBookMask * flipbookAlpha);
    }
    ...
    float applyFlipbookAlphaToShadow(float2 uv)
    {
        ...
        if (float(0))
        {
            ...
            return flipbookColor.a * float4(0.3229691,0.3229691,0.3229691,1).a;
        }
        return 1;
    }
