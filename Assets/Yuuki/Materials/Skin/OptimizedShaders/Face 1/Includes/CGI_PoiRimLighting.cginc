...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float viewDotNormal = saturate(abs(dot(poiCam.viewDir, poiMesh.normals[float(0)])));
    ...
    float rimBrighten = float(0.53);

    float rimWidth = lerp( - .05, 1, float(0.62));
    #ifdef POI_AUDIOLINK
    ...
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.250646,0.1281221,0.1281221,1);
    #else
        rimColor = float4(0.250646,0.1281221,0.1281221,1);
    #endif
    ...
    rim = 1 - smoothstep(min(float(0.063), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0.250646,0.1281221,0.1281221,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.893)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.893)) + lerp(albedo.rgb, rimColor, float(0.893)) * rimBrighten, rim);
}
#endif
