...
half2 _AudioLinkFlipbookFrame;
    ...
    void applyFlipbook(inout float4 finalColor, inout float3 flipbookEmission)
    {
            ...
            flipBookMask = 1;
        #endif
        float4 flipbookScaleOffset = float4(0.41,0.38,0.24,0.14);
        ...
        if (float(0) == 0)
        {
        ...
        #if defined(PROP_FLIPBOOKTEXARRAY) || !defined(OPTIMIZER_ENABLED)
            float currentFrame = fmod(float(-1), float(90));
            if (float(-1) < 0)
            {
                currentFrame = (_Time.y / (1 / float(10))) % float(90);
            }
            #ifdef POI_AUDIOLINK
            ...
            if (float(0))
            {
                float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(90))));
        ...
        float flipbookEmissionStrength = float(11);
        #ifdef POI_AUDIOLINK
