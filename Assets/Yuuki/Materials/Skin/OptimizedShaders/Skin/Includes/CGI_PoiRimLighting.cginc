...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimBrighten = float(0.88);

    float rimWidth = lerp( - .05, 1, float(0.966));
    #ifdef POI_AUDIOLINK
    ...
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.130239,0.06134963,0.06134963,1);
    #else
        rimColor = float4(0.130239,0.06134963,0.06134963,1);
    #endif
    ...
    rim *= float4(0.130239,0.06134963,0.06134963,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.541)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.541)) + lerp(albedo.rgb, rimColor, float(0.541)) * rimBrighten, rim);
}
#endif
