...
half2 _AudioLinkFlipbookFrame;
    ...
    void applyFlipbook(inout float4 finalColor, inout float3 flipbookEmission)
    {
            ...
            flipBookMask = 1;
        #endif
        float4 flipbookScaleOffset = float4(0.075,0.105,0.275,0.195);
        ...
        if (float(0) == 0)
        {
        ...
        #if defined(PROP_FLIPBOOKTEXARRAY) || !defined(OPTIMIZER_ENABLED)
            float currentFrame = fmod(float(-1), float(12));
            if (float(-1) < 0)
            {
                currentFrame = (_Time.y / (1 / float(6))) % float(12);
            }
            #ifdef POI_AUDIOLINK
            ...
            if (float(0))
            {
                float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(12))));
        ...
        finalColor.rgb = finalColor + flipBookPixel.rgb * float(0.25) * flipBookMask * flipbookAlpha;
        finalColor.rgb = finalColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * flipBookMask * float(0.46) * flipbookAlpha);
